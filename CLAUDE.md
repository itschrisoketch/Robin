...
- **Prompt-first intake → ranked cards, rendered instantly (NOT a chat thread).** The main page (`app/page.tsx`) is a single centered column: quick-start persona chips + a 20px-radius composer (free-text `goals` + inline `repo` field + up-arrow submit), with the structured fields (`languages`/`years`/`interests`/`hours`) collapsed behind an "Add details" disclosure in `Composer`. There is no streaming conversation thread — results replace the empty space below the composer. (DESIGN.md/PRODUCT.md predate all of this; they're stale on UI specifics.)
...
- **Theme: light, single Bitcoin-orange accent (locked).** Color tokens live in `app/globals.css` — one cool-neutral grey scale; `--color-robin` (orange) is for **fills only**, `--color-robin-deep` for orange **text/glyphs** (orange fails AA as text on white), and the `--color-honey*` set is the same accent applied as the redirect "signpost" treatment, not a second hue. Fonts: **Bricolage Grotesque** (display/voice, `.voice`/`.font-display`), **Geist** (UI/body, via the `geist` package), **Geist Mono** (metadata). The extension bundles Geist via `@font-face` in `extension/robin.css`.
